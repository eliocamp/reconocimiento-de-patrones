...
La Figura \ref{fig:cv-fig} muestra la tasa de acierto (proporción de observaciones correctamente categorizadas) computada por validación cruzada para distintos valores de $\lambda$ y $\gamma$. 
...
```{r cv-fig, fig.cap = "Acierto computado a partir de validación cruzada para distintos valores de $\\lambda$ y $\\gamma$"}
positive[, mean(V1), by = .(lambda, gamma)] %>% 
  copy() %>% 
