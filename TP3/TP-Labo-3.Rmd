...
Luego, extiendo el algoritmo PEGASOS para poder usar un kernel arbitrario. En la Figura \ref{fig:svm-gauss} se muestran los resultados de applicar SVM con kernel gaussiano con distintos valores de $\gamma$ y $\lambda$ y un $\delta = 2$. 
...
models_k <- readRDS(here::here("TP3", "models_r.Rds"))
sims <- readRDS(here::here("TP3", "sims.Rds"))
fields <- readRDS(here::here("TP3", "fields.Rds"))
...
bishop <- fread(here::here("TP3", "bishiop7.2.csv")) %>% 
  ...
  na.omit() 
bishop_svm <- SVM(id ~ x + y, data = bishop, lambda = 0.1, epochs = 300,
